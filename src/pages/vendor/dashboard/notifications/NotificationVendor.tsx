...
const NotificationVendor: React.FC = () => {
    ...
    const { data, isLoading, isError, refetch } = useVendorNotifications(page, limit);
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                        {isLoading ? (
                            ...
                            <div className="p-12 flex flex-col items-center justify-center text-center">
                                ...
                                <h3 className="text-lg font-semibold text-slate-800">Oops! Something went wrong</h3>
                                <p className="text-slate-500 mb-6">We couldn't fetch your notifications.</p>
                                <button
                                    onClick={() => refetch()}
                            ...
                            <div className="p-16 flex flex-col items-center justify-center text-center">
                                ...
                                <p className="text-slate-500 max-w-xs mx-auto mt-2">
                                    When you receive updates about loans, payments, or messages, they'll appear here.
                                </p>
                            </div>
                        ) : (
                            <div className="divide-y divide-slate-100">
                                {notifications.map((notification) => (
                                    <div
                                        ...
                                        className={`p-5 flex gap-4 cursor-pointer transition-all hover:bg-slate-50 ${!notification.isRead ? "bg-teal-50/30 border-l-4 border-teal-500" : "border-l-4 border-transparent"
                                            }`}
                                    >
                                        <div className={`mt-1 flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${!notification.isRead ? "bg-teal-100 text-teal-600" : "bg-slate-100 text-slate-500"
                                            }`}>
                                            <Bell size={18} />
                                        </div>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between gap-4 mb-1">
                                                <h4 className={`text-sm font-semibold truncate ${!notification.isRead ? "text-slate-900" : "text-slate-600"
                                                    }`}>
                                                    {notification.title}
                                                </h4>
